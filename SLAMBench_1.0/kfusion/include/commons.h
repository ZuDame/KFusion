...
inline bool is_file(std::string path) {
  ...
  fopen_s(&file_test, path.c_str(), "rb");

  if (file_test == NULL)
    return false;

  size_t one_read = fread(&b, 1, 1, file_test);
...
template <typename T> std::string NumberToString(T Number, int width = 6) {
  std::ostringstream ss;
  ss << std::setfill('0') << std::setw(width) << Number;
  return ss.str();
}

template <typename T> void read_input(std::string inputfile, T *in) {
  size_t isize;
...
inline float sq(float r) { return r * r; }
...
struct Volume {
  ...
  float v(const uint3 &pos) const { return operator[](pos).x; }
  ...
  float interp(const float3 &pos) const {
    ...
    return (((vs2(lower.x, lower.y, lower.z) * (1 - factor.x) +
              vs2(upper.x, lower.y, lower.z) * factor.x) *
                 (1 - factor.y) +
             (vs2(lower.x, upper.y, lower.z) * (1 - factor.x) +
              vs2(upper.x, upper.y, lower.z) * factor.x) *
                 factor.y) *
                (1 - factor.z) +
  ...
  float3 grad(const float3 &pos) const {
    ...
    gradient.x = (((vs2(upper_lower.x, lower.y, lower.z) -
                    vs2(lower_lower.x, lower.y, lower.z)) *
                       (1 - factor.x) +
                   (vs2(upper_upper.x, lower.y, lower.z) -
                    vs2(lower_upper.x, lower.y, lower.z)) *
                       factor.x) *
                      (1 - factor.y) +
                  ((vs2(upper_lower.x, upper.y, lower.z) -
                    vs2(lower_lower.x, upper.y, lower.z)) *
                       (1 - factor.x) +
                 ...
                 (((vs2(upper_lower.x, lower.y, upper.z) -
                    vs2(lower_lower.x, lower.y, upper.z)) *
                       (1 - factor.x) +
    ...
    gradient.y = (((vs2(lower.x, upper_lower.y, lower.z) -
                    vs2(lower.x, lower_lower.y, lower.z)) *
                       (1 - factor.x) +
                   (vs2(upper.x, upper_lower.y, lower.z) -
                    vs2(upper.x, lower_lower.y, lower.z)) *
                       factor.x) *
                      (1 - factor.y) +
                  ((vs2(lower.x, upper_upper.y, lower.z) -
                    vs2(lower.x, lower_upper.y, lower.z)) *
                       (1 - factor.x) +
                 ...
                 (((vs2(lower.x, upper_lower.y, upper.z) -
                    vs2(lower.x, lower_lower.y, upper.z)) *
                       (1 - factor.x) +
    ...
    gradient.z = (((vs2(lower.x, lower.y, upper_lower.z) -
                    vs2(lower.x, lower.y, lower_lower.z)) *
                       (1 - factor.x) +
                   (vs2(upper.x, lower.y, upper_lower.z) -
                    vs2(upper.x, lower.y, lower_lower.z)) *
                       factor.x) *
                      (1 - factor.y) +
                  ((vs2(lower.x, upper.y, upper_lower.z) -
                    vs2(lower.x, upper.y, lower_lower.z)) *
                       (1 - factor.x) +
                 ...
                 (((vs2(lower.x, lower.y, upper_upper.z) -
                    vs2(lower.x, lower.y, lower_upper.z)) *
                       (1 - factor.x) +
...
inline void writeVolume(std::string filename, Volume v) {

...
void computeFrame(Volume &integration, float3 *vertex, float3 *normal,
                  TrackData *trackingResult, Matrix4 &pose,
                  const float *inputDepth, const uint2 inputSize,
                  const float *gaussian, const std::vector<int> iterations,
                  float4 k, const uint frame);
...
public:
  ...
  // Allow a kfusion object to be created with a pose which include orientation
  // as well as position
  Kfusion(uint2 inputSize, uint3 volumeResolution, float3 volumeDimensions,
  ...
  float3 getPosition() {
    ...
    // _initPose.z << "    ";
    // std::cerr << "pose =" << pose.data[0].w << "," << pose.data[1].w  <<","
    // << pose.data[2].w << "    ";

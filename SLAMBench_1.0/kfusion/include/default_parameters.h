...
static std::string short_options = "a:qc:d:f:i:l:m:k:o:p:r:s:t:v:y:z:";

static struct option long_options[] = {
	{ "track-threshold", required_argument, 0, 'a' },
...
struct Configuration {
  ...
  inline void print_arguments() {
	...
	std::cerr << "-a  (--track-threshold)   : default is "
		<< track_threshold_default << std::endl;
  }
  ...
  Configuration(unsigned int argc, char **argv) {
	...
	track_threshold = track_threshold_default;
    ...
    while ((c = getopt_long(argc, argv, short_options.c_str(), long_options,
      ...
      switch (c) {
	  case 'a':
		  this->track_threshold = atof(optarg);
		  std::cerr << "update track_threshold to " << this->track_threshold
			  << std::endl;
		  break;
      case 'b':
      ...
      case 'c': //   -c  (--compute-size-ratio)
        ...
        std::cerr << "update compute_size_ratio to " << this->compute_size_ratio
                  << std::endl;
      ...
      case 'd':
        ...
        std::cerr << "update dump_volume_file to " << this->dump_volume_file
                  << std::endl;
        break;
      case 'f': //   -f  (--fps)
      ...
      case 'i': //   -i  (--input-file)
        ...
        std::cerr << "update input_file to " << this->input_file << std::endl;
        {
          ...
          fopen_s(&input_f, this->input_file.c_str(), "rb");
        ...
        /*struct _stat st;
        if (_stat(this->input_file.c_str(), &st) != 0) {
          std::cerr << "ERROR: --input-file (-i) does not exist (was "
                    << this->input_file << ")\n";
          flagErr++;
        }*/
      ...
      case 'r': //   -r  (--integration-rate)
        ...
        std::cerr << "update integration_rate to " << this->integration_rate
                  << std::endl;
      ...
      case 's': //   -s  (--map-size)
        ...
        std::cerr << "update map_size to " << this->volume_size.x << "mx"
                  << this->volume_size.y << "mx" << this->volume_size.z << "m"
                  << std::endl;
        if ((this->volume_size.x <= 0) || (this->volume_size.y <= 0) ||
            (this->volume_size.z <= 0)) {
          std::cerr << "ERROR: --volume-size (-s) all dimensions must > 0 (was "
                    << optarg << ")\n";
          flagErr++;
        }
        break;
      ...
      case 'v': //   -v  (--volumetric-size)
        ...
        std::cerr << "update volumetric_size to " << this->volume_resolution.x
                  << "x" << this->volume_resolution.y << "x"

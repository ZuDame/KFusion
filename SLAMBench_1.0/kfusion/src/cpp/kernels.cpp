  ...
  {                                                                            \
    if (print_kernel_timing) {                                                 \
      ...
      if ((tock_clockData.tv_sec > tick_clockData.tv_sec) &&                   \
        ...
        std::cerr << tock_clockData.tv_sec - tick_clockData.tv_sec             \
...
float track_threshold = track_threshold_default;
// input once
float *gaussian;
...
Kfusion::Kfusion(uint2 inputSize, uint3 volumeResolution,
                 float3 volumeDimensions, float3 initPose,
...
Kfusion::Kfusion(uint2 inputSize, uint3 volumeResolution,
                 float3 volumeDimensions, Matrix4 initPose,
                 std::vector<int> &pyramid)
    : computationSize(make_uint2(inputSize.x, inputSize.y)) {
...
void Kfusion::languageSpecificConstructor() {
  ...
  floatDepth =
      (float *)calloc(sizeof(float) * computationSize.x * computationSize.y, 1);
      ...
      for (int i = -r; i <= r; ++i) {
        for (int j = -r; j <= r; ++j) {
          ...
          if (curPix > 0) {
            ...
            const float factor =
                gaussian[i + r] * gaussian[j + r] * expf(-mod / e_d_squared_2);
            t += factor * curPix;
            sum += factor;
          }
        }
  ...
  for (y = 0; y < imageSize.y; y++) {
    for (x = 0; x < imageSize.x; x++) {
      ...
      if (depth[x + y * imageSize.x] > 0) {
        vertex[x + y * imageSize.x] =
            depth[x + y * imageSize.x] * (rotate(invK, make_float3(x, y, 1.f)));
      } else {
...
void new_reduce(int blockIndex, float *out, TrackData *J, const uint2 Jsize,
  ...
  float sums0, sums1, sums2, sums3, sums4, sums5, sums6, sums7, sums8, sums9,
      sums10, sums11, sums12, sums13, sums14, sums15, sums16, sums17, sums18,
      sums19, sums20, sums21, sums22, sums23, sums24, sums25, sums26, sums27,
      sums28, sums29, sums30, sums31;
  sums0 = 0.0f;
  sums1 = 0.0f;
  sums2 = 0.0f;
  sums3 = 0.0f;
...
#pragma omp parallel for reduction(                                            \
    + : sums0, sums1, sums2, sums3, sums4, sums5, sums6, sums7, sums8, sums9,  \
    sums10, sums11, sums12, sums13, sums14, sums15, sums16, sums17, sums18,    \
    sums19, sums20, sums21, sums22, sums23, sums24, sums25, sums26, sums27,    \
    sums28, sums29, sums30, sums31)
...
#pragma omp parallel for private(blockIndex)
  ...
  for (blockIndex = 0; blockIndex < 8; blockIndex++) {
    ...
    for (int threadIndex = 0; threadIndex < 112; threadIndex++) {
      ...
      // WE NO LONGER NEED TO DO THIS AS the threads execute sequentially inside
      // a for loop
    ...
    for (int ssline = 0; ssline < 32;
         ssline++) { // sum up columns and copy to global memory in the final 32
                     // threads
...
#pragma omp parallel for shared(output), private(pixel, pixelx, pixely)
  for (pixely = 0; pixely < inSize.y; pixely++) {
    for (pixelx = 0; pixelx < inSize.x; pixelx++) {
      ...
      const float3 diff =
          refVertex[refPixel.x + refPixel.y * refSize.x] - projectedVertex;
      ...
      float dp = dot(projectedNormal, referenceNormal);
  ...
  for (y = 0; y < normalSize.y; y++)
    for (unsigned int x = 0; x < normalSize.x; x++) {
      ...
      if (n.x == -2) {
        ...
        n = normalize(n);
        out[pos] =
            make_uchar3(n.x * 128 + 128, n.y * 128 + 128, n.z * 128 + 128);
      }
    }
  ...
  for (y = 0; y < depthSize.y; y++) {
    ...
    for (unsigned int x = 0; x < depthSize.x; x++) {
      ...
      if (depth[pos] < nearPlane)
        out[pos] = make_uchar4(
            255, 255, 255,
            0); // The forth value is a padding in order to align memory
      else {
...
#pragma omp parallel for shared(out), private(y)
  for (y = 0; y < depthSize.y; y++) {
    for (unsigned int x = 0; x < depthSize.x; x++) {
      ...
      float4 hit = raycast(volume, make_uint2(x, y), view, nearPlane, farPlane,
                           step, largestep);
      if (hit.w > 0) {
        ...
        if (length(surfNorm) > 0) {
          ...
          out[pos] =
              make_uchar4(col.x, col.y, col.z,
                          0); // The forth value is a padding to align memory
        } else {
          out[pos] = make_uchar4(
              0, 0, 0, 0); // The forth value is a padding to align memory
        }
      } else {
        out[pos] = make_uchar4(
            0, 0, 0, 0); // The forth value is a padding to align memory
...
bool Kfusion::tracking(float4 k, float icp_threshold, uint tracking_rate,
  ...
  for (unsigned int i = 1; i < iterations.size(); ++i) {
    halfSampleRobustImageKernel(
        ...
        make_uint2(computationSize.x / (int)pow(2, i - 1),
                   computationSize.y / (int)pow(2, i - 1)),
        e_delta * 3, 1);
  }
  ...
  //{
  //  FILE *ref_file = NULL;
  //  ref_file = fopen("D:\\movidius\\MATLAB\\Depth\\SLAM\\reference.dat",
  //  "wb");

  //  if (ref_file) {
  //    fwrite(vertex, sizeof(float3), 640 * 480, ref_file);
  //    fwrite(normal, sizeof(float3), 640 * 480, ref_file);
  //    fclose(ref_file);
  //  }
  //}

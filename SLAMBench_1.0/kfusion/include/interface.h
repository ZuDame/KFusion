...
public:
  ...
  inline bool readNextDepthFrame(uchar3 *, unsigned short int *depthMap) {
    ...
    float *FloatdepthMap = (float *)malloc(_size.x * _size.y * sizeof(float));
  ...
  inline bool readNextDepthFrame(float *depthMap) {
    ...
    get_next_frame();
    filename << this->_dir << "/scene_00_" << std::setfill('0') << std::setw(4)
             << _frame << ".depth";
    ...
    for (int v = 0; v < _sceneheight; v++) {
      for (int u = 0; u < _scenewidth; u++) {
        ...
        depthMap[u + v * _scenewidth] =
            depthMap[u + v * _scenewidth] /
            std::sqrt(u_u0_by_fx * u_u0_by_fx + v_v0_by_fy * v_v0_by_fy + 1);
      }
    }
    return index > 0;
  }
...
public:
  ...
  inline bool readNextDepthFrame(uchar3 *raw_rgb,
    ...
    if (depthMap) {
      ...
      fseek(_pFile,
            newImageSize[0] * newImageSize[1] * sizeof(unsigned short int),
            SEEK_CUR);
      expected_size += 1 + newImageSize[0] * newImageSize[1];
    }
    ...
    if (raw_rgb) {
      total += fread(&(newImageSize), sizeof(newImageSize), 1, _pFile);
      total += fread(raw_rgb, sizeof(uchar3), newImageSize[0] * newImageSize[1],
                     _pFile);
      expected_size += 1 + newImageSize[0] * newImageSize[1];
    } else {
      total += fread(&(newImageSize), sizeof(newImageSize), 1, _pFile);
...
public:
  ...
  OpenNIDepthReader(std::string filename, int fps, bool blocking_read)
    ...
    if (colorMode.getResolutionX() != _size.x ||
        colorMode.getResolutionY() != _size.y) {
